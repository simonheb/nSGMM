...
bool BBP_update_BR_analytically_cpp_fast8_smarter(uword i, mat & transfers, vec & net_transfers_in, vec & net_transfers_out, const vec& income, const mat& altruism, const mat& capacities) {
...
bool BBP_update_BR_analytically_cpp_fast9(uword i, mat & transfers, vec & net_transfers_in, vec & net_transfers_out, const vec& income, const mat& altruism, const mat& capacities) {
  ...
  uword nextrecipient = max_ind_simon(marginal_util);
  //std::cout << endl<<"nextrecipient:" << nextrecipient<<endl;
  ...
  for(uword giveto=1; giveto<income.n_elem; ++giveto) {  //if people have many transactions, it might make sense to not go through the first cases, this would however require a full ranking first
    ...
    marginal_util(nextrecipient)=-datum::inf;
    //std::cout << "nextrecipient:" << nextrecipient<<endl;
    ...
    nextT=(nextC-nextA*(isconsumptionnetofmaxtransfers))/(-1-nextA);
    double nexttransfertolastguy = (altruism(i-1,nextrecipient))*(isconsumptionnetofmaxtransfers-nextT)-consumption(nextrecipient); // 
    //std::cout << "would give that one:" << nexttransfertolastguy<<endl;
    
    if (nexttransfertolastguy>0) {continue;}
    uvec whotooptimize=whotogiveto.head(giveto);
    //std::cout << "optimizing" << whotooptimize << endl;
    double CC = C; //the one we use within this loop
    double AA = A; //-"- here we take out some obs every round
  ...
  for (r=0;r<30000;r++) {
    ...
    for (int rep=0;rep<20;rep++){
      ...
      for (uword u=0; u<updatingmax; u++) { //for each player
        uword i=updating(u);        //find the best response
        //std::cout << i;
        bool updated=BBP_update_BR_analytically_cpp_fast9(i,transfers,net_transfers_in,net_transfers_out,income,altruism,capacity);
        //update transfers
        if (updated) {//!approx_equal(transfers.row(i-1),oldt,"absdiff",0.00001)) {
          updates=updates+1;
          updating(next_updateingmax++)=i;
